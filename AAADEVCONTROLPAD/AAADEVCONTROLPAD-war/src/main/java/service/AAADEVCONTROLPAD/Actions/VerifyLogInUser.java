...
import service.AAADEVCONTROLPAD.Http.GetFileAccess;
import service.AAADEVCONTROLPAD.Security.XSSPrevent;
    ...
    public JSONObject verify() throws IOException, ServletException {
        ...
        if (mat.find()) {
            if (validateUserAndPass(XSSPrevent.stripXSS(emailString), decryptText(passString, "secret"), request)) {
                ...
                System.out.println("El usuario no se ha encontrado dentro del archivo");
                return new JSONObject().put("status", "error");
            }
        } else {
            System.out.println("El email no es un formato válido");
            return new JSONObject().put("status", "error");
        }
    }
